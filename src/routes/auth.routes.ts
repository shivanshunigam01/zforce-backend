...
import { forgotPasswordSchema, loginSchema, refreshSchema, resetPasswordSchema } from "../validators/auth.validators";
...
router.post("/admin/login", validate(loginSchema), async (req, res, next) => {
  try { await loginByRole(req, res, "super_admin"); } catch (e) { next(e); }
});
...
router.get("/dealer/me", authJwt(["dealer", "super_admin"]), async (req, res, next) => {
  try {
    const user = await User.findById(req.user!.sub);
    res.json({ data: toJSON(user) });
  } catch (e) { next(e); }
